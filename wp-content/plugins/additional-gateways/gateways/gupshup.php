...
class GupShup
{
    ...
    public static function process_sms($api_key, $sender, $messages, $test_call)
    {
        ...
        foreach ($chunked_messages as $message_batch) {
            ...
            curl_setopt($curl, CURLOPT_URL, 'https://api.gupshup.io/sm/api/ent/sms/msg?' . $encoded_query);
    ...
    public static function process_whatsapp($gateway_fields, $mobile, $message, $testCall)
    {
        ...
        $src_name = $gateway_fields['src_name'];

        $data = array(
            'message' => json_encode(array('isHSM' => true, 'type' => 'text', 'text' => $message)),
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://api.gupshup.io/sm/api/v1/msg?' . $query);
        $result = curl_exec($ch);
        curl_close($ch);
